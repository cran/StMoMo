...
bootstrap.fitStMoMo <- function(object, nBoot = 1, type = c("semiparametric", "residual"),
  ...
  if (link == "log") {
    if (type == "residual"){
      bootSamples <- genPoissonResBootSamples(devRes = residuals(object, scale = FALSE),
    ...
    if (type == "residual"){
      bootSamples <- genBinomialResBootSamples(devRes = residuals(object, scale = FALSE),
                                          dhat = fitted(object, type = "deaths"),
                                          E = object$Ext, nBoot)
    } else if (type == "semiparametric"){
      if (deathType == "observed"){
      ...
      bootSamples <- genBinomialSemiparametricBootSamples(D = D, E = object$Ext, nBoot)      
    }
  }
  
  #Fit the model to each of 
  refit <- function(Dxt) {
    getMinimalFitStMoMo(fit(object = object$model, Dxt = Dxt, Ext = object$Ext, ages = object$ages, years = object$years,
        oxt = object$oxt, wxt = object$wxt, start.ax = object$ax, start.bx = object$bx, 
        start.kt = object$kt, start.b0x = object$b0x, start.gc = object$gc, verbose = FALSE))
...
#' @inheritParams bootstrap
#' 
#' @return a list of length \code{nBoot} of marices with matching sampled deaths
#' 
#' @references
#' Renshaw, A. E., & Haberman, S. (2008). On simulation-based approaches to risk measurement in 
#' mortality with specific reference to Poisson Lee-Carter modelling. 
...
binomRes2q<-function(dhat,e,res){
  ...
  if (is.na(dhat) || is.na(e)) return(NA)
  if (e <= 0) return(0L)
  a <- log(dhat / (e - dhat))
...
#' @inheritParams bootstrap
#' 
#' @return a list of length \code{nBoot} of matrices with matching sampled deaths
#' 
#' @references
#' Debon, A., Martinez-Ruiz, F., & Montes, F. (2010). A geostatistical approach for dynamic 
#' life tables: The effect of mortality on remaining lifetime and annuities. 
...
#' 
#' Generate Binomial semiparametric bootstrap samples using a suitable adaptation of
#' the Poisson procedure described in Brouhns et al (2005).
#' 
#' @param D matrix of deaths
