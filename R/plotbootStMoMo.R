...
#' 
#' @usage
#' \method{plot}{bootStMoMo}(x, nCol = 2, parametricbx = TRUE, colour = rgb(0, 0, 0),
    ...
    for (i in 1:N){      
      ...
      if (parametricbx == TRUE || is.nonparametric(x$model$model$periodAgeFun[[i]])) {
        ...
        plotParameterFan(x = ages, y = bx[, i], yBoot = bxBoot, 
                         main = substitute(paste(beta[x]^{(i)}, " vs. x", ""), list(i = i)),
                         xlab = "age", ylab = "", probs = probs, 
                         fan.col = fan.col, n.fan = n.fan, ...)
      ...
      plotParameterFan(x = years, y = kt[i, ], yBoot = ktBoot, 
                       main = substitute(paste(kappa[t]^{(i)}, " vs. t", ""), list(i = i)),
...
#' 
#' @details In order for the plotting to look appropiately the intervals of the data with
#' non missing values need to be found. Otherwise fanplot::fan doesn't work.
#' 
#' @keywords internal
plotParameterFan <- function(x, y, yBoot, main, xlab, ylab, probs, fan.col, n.fan, ...){  
  plot(NULL, ylab = ylab, xlab = xlab, main = main,
       xlim = range(x), ylim = range(yBoot, finite = TRUE))
  ...
  while (i_end < n){
    ...
    if (i_start != i_end){
      fanplot::fan(pp[, i_start:i_end], data.type = "values", start = x[i_start], probs = probs, fan.col = fan.col, 
                   n.fan = n.fan, ln = NULL)      
