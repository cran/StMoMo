...
residuals.fitStMoMo<-function(object, scale = TRUE, ...){
  D <- object$Dxt+0.00001 #Add a small amount to compesate for the possibility of 0 deaths
  W <- object$wxt
  ind <- (W > 0)
  res <- array(NA, dim(W))
  ...
  if(scale) phi <- sum(res[ind]) / (object$nobs - object$npar)
  else phi <- 1
...
#' of the residuals.
#' 
#' @usage \method{plot}{resStMoMo}(x, type = c("scatter", "colourmap", "signplot"), 
#' reslim = range(x$residuals, na.rm = TRUE), plotAge = TRUE, plotYear = TRUE, 
#' plotCohort  = TRUE, pch = 20, ...)
#' 
#' @param x an object of class \code{resStMoMo} with the residuals of a 
#' Stochastic Mortality Model.
#' @param type the type of the plot. The alternatives are \code{"scatter"}(default)
#' \code{"colourmap"}, and \code{"signplot"}.
#' @param reslim optional numeric vector of length 2, giving the  range of the residuals.
...
#' year and cohort (year of birth) are produced. 
#'
#' When \code{type = "colourmap"} a two dimensional colour map of the residuals is 
#' plotted. This is produced using function \code{\link[fields]{image.plot}}. See 
#' \code{\link[fields]{image.plot}} for further parameters that can be passed to this
#' type of plots.
#'
...
scatterplotAPC <- function(mat, ages, years, plotAge = TRUE, plotYear = TRUE, 
  ...
  # Do a scatter plot of the data in a data frame by age - period and cohort.
  # The main use of this is to do plots of model residuals
  #
  #            
  #  ARGUMENT
  #
  ...
  #if (is.null(ylim)) ylim = range(data$y, finite = TRUE)
