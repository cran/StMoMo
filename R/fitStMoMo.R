...
#'
#' \code{fit} is a generic function which means that new fitting strategies can be added
#' for particular stochastic mortality models. See for instance \code{\link{fit.StMoMo}}.
#'
#' @param object an object used to select a method. Typically of class \code{StMoMo} or 
...
#' @param ages.fit optional vector of ages to include in the fit. Must be a subset of
#' \code{ages}. 
#' @param years.fit optional vector of years to include in the fit. Must be a subset of
#' \code{years}. 
...
fit.StMoMo <- function(object, Dxt, Ext, ages = 1:nrow(Dxt), years = 1:ncol(Dxt), 
                       ...
                       wxt = NULL, start.ax = NULL, start.bx = NULL,
                       start.kt = NULL,
                       start.b0x = NULL,
                       start.gc = NULL,
                       verbose = TRUE, ...){
  ...
  if( is.null(oxt) ){
    ...
    rownames(oxt) <- ages
    colnames(oxt) <- years
    fitDataO<- (reshape2::melt(oxt, value.name = "o", varnames = c("x", "t")))
  ...
  if (verbose) {
    if (length(zeroWeigthAges) > 0) {
      cat("StMoMo: The following ages have been zero weigthed:", zeroWeigthAges, "\n")
    }
    if (length(zeroWeigthYears) > 0) {
      cat("StMoMo: The following years have been zero weigthed:", zeroWeigthYears, "\n")
    }
    if (length(zeroWeigthCohorts) > 0) {
      cat("StMoMo: The following cohorts have been zero weigthed:", zeroWeigthCohorts, "\n")
    }    
  }  
  
  ### Set starting values
  if (object$link == "logit") {        
    coefNames <- gnm(formula = as.formula(object$gnmFormula), data = fitData, family = binomial(link = "logit"), 
                        weights = fitData$E * fitData$w, start = start, method = "coefNames")
  } else {        
    coefNames <- gnm(formula = as.formula(object$gnmFormula), data = fitData, family = poisson(link = "log"), 
                        weights = fitData$w, start = start, method = "coefNames")
  }  
  ...
  startCoef <- processStartValues(object, coefNames, start.ax, start.bx, start.kt, start.b0x, start.gc, 
                                 ages, years, cohorts)
  ...
  if (object$link == "logit") {        
    fittingModel <- gnm(formula = as.formula(object$gnmFormula), data = fitData, family = binomial(link = "logit"), 
                        weights = fitData$E * fitData$w, start = startCoef, verbose = verbose, ...)
  } else {        
    fittingModel <- gnm(formula = as.formula(object$gnmFormula), data = fitData, family= poisson(link = "log"), 
                        weights = fitData$w, start = startCoef, verbose = verbose, ...)
  }  
  if (verbose) cat("StMoMo: Finish fitting with gnm\n")  
  fail <- is.null(fittingModel$conv)
  if (fail) {
    conv <- FALSE
    warning("StMoMo: The model fitting failed and no model could be estimated.\n")      
    out <- list(model = object, ax = NULL, bx = NULL, kt = NULL, b0x = NULL, gc = NULL, 
                Dxt = Dxt, Ext = Ext,
                oxt = oxt , wxt = wxt, ages = ages, years = years, cohorts = cohorts,
                fittingModel = fittingModel, loglik = NULL,deviance = NULL, npar = NULL, 
                nobs = NULL, conv = conv, fail = fail, fitData = fitData )          
    class(out) <- "fitStMoMo"
    return(out)    
  }
  else {
  ...
  fittedCoef <- extractCoefficientsFromGnm(object, coef(fittingModel), ages, years, cohorts, 
                                           zeroWeigthAges, zeroWeigthYears, zeroWeigthCohorts)  
  
  ### Apply identifiability constraints
  
  oldLinkPred <- predictLink(fittedCoef$ax, fittedCoef$bx, fittedCoef$kt, fittedCoef$b0x, 
                             fittedCoef$gc, oxt, ages, years)
  constPar<- object$constFun(fittedCoef$ax, fittedCoef$bx, fittedCoef$kt, fittedCoef$b0x, fittedCoef$gc, 
                             wxt, ages)
  ax <- constPar$ax
  bx <- constPar$bx
  kt <- constPar$kt
  ...
  NAinNewParameters <- (!is.null(fittedCoef$ax) && (is.null(ax) || anyNA(ax[!is.na(fittedCoef$ax)])))  || 
                       (!is.null(fittedCoef$bx) && (is.null(bx) || anyNA(bx[!is.na(fittedCoef$bx)])))  ||
                       (!is.null(fittedCoef$kt) && (is.null(kt) || anyNA(kt[!is.na(fittedCoef$kt)])))  ||     
                       (!is.null(fittedCoef$b0x) && (is.null(b0x) || anyNA(b0x[!is.na(fittedCoef$b0x)])))  ||   
                       (!is.null(fittedCoef$gc) && (is.null(gc) || anyNA(gc[!is.na(fittedCoef$gc)])))
  ...
  if (object$link == "logit"){
    ...
    qhatxt <- temp/(temp + 1)
    loglik <- computeLogLikBinomial(obs = Dxt / Ext, 
                                    fit = qhatxt,
                                    exposure = Ext, weight = wxt)
    deviance <- computeDevianceBinomial(obs = Dxt / Ext, 
                                        fit = qhatxt,
                                        exposure = Ext, weight = wxt)      
...
#' @param zeroWeigthAges character vector of years whose parameters cannot be
#' estimated because all data is zero weighted
#' @param zeroWeigthYears character vector of years whose parameters cannot be
#' estimated because all data is zero weighted
#' @param zeroWeigthCohorts character vector of cohort whose parameters cannot be
#' estimated because all data is zero weighted
#' 
#' @details Weigth vectors wx, wx, wc are used to identify parameters that cannot
#' be estimated because all the data is weighted out.
#' 
#' @return A list with the model parameters, ax, bx, kt, b0x, gc
#' @keywords internal
extractCoefficientsFromGnm <- function(object,coefGnmModel, ages, years, cohorts, 
                                       zeroWeigthAges, zeroWeigthYears, zeroWeigthCohorts) {
  ...
  if (object$staticAgeFun == TRUE) {
    axTemp <- coefGnmModel[grep(pattern = "^factor[(]x[)]", names(coefGnmModel))]    
    names(axTemp)<-sub(pattern = "factor[(]x[)]", replacement="" , x = names(axTemp))
    ax <- rep(NA,nAges)
    names(ax) <- ages
  ...
  if (N > 0) {    
    ...
    for (i in 1:N) {
      if (is.function(object$periodAgeFun[[i]])) {
        ...
        names(ktTemp) <- sub(pattern = "factor[(]t[)]", replacement = "" , x = names(ktTemp))
        names(ktTemp) <- sub(pattern = paste("B", i, sep = ""), replacement = "" , x = names(ktTemp))
        names(ktTemp) <- sub(pattern = ":", replacement = "" , x = names(ktTemp))
        ...
        pattern <- paste("Mult[(]., factor[(]t[)], inst = ",inst,"[)].factor[(]x[)]", sep = "")
        bxTemp <- coefGnmModel[grep(pattern = pattern, names(coefGnmModel))]
        names(bxTemp) <- sub(pattern = pattern, replacement = "" , x = names(bxTemp))
        bx[names(bxTemp), i] <- bxTemp
        bx[zeroWeigthAges, i] <- NA 
        #kt
        pattern <- paste("Mult[(]factor[(]x[)], ., inst = ", inst, "[)].factor[(]t[)]", sep = "")
        ktTemp <- coefGnmModel[grep(pattern = pattern, names(coefGnmModel))] 
        names(ktTemp) <- sub(pattern = pattern, replacement = "" , x = names(ktTemp))
        ktTemp[is.na(ktTemp)] <- 0
        kt[i, names(ktTemp)] <- ktTemp      
        kt[i, zeroWeigthYears] <- NA 
      }      
  ...
  if ( !is.null(object$cohortAgeFun) == TRUE ) {
    ...
    if (is.function(object$cohortAgeFun)) {
      ...
      gcTemp <- coefGnmModel[c(grep(pattern = pattern1, names(coefGnmModel)),
                               grep(pattern = pattern2, names(coefGnmModel)))] 
      names(gcTemp) <- sub(pattern = "factor[(]c[)]", replacement = "" , x = names(gcTemp))
      ...
      names(b0xTemp) <- sub(pattern = pattern, replacement = "" , x = names(b0xTemp))
      b0x[names(b0xTemp)] <- b0xTemp
      b0x[zeroWeigthAges] <- NA
      #gc
      pattern <- "Mult[(]factor[(]x[)], .[)].factor[(]c[)]"
      gcTemp <- coefGnmModel[grep(pattern = pattern, names(coefGnmModel))] 
      names(gcTemp) <- sub(pattern = pattern, replacement = "" , x = names(gcTemp))
      gcTemp[is.na(gcTemp)] <- 0
      gc[names(gcTemp)] <- gcTemp
      gc[zeroWeigthCohorts] <- NA
...
#' 
#' Convert the initial parameters supplied by the user into parameters suitable for 
#' use within \code{gnm}
#' 
#' @param object an object of class \code{"StMoMo"} defining the stochastic
#' mortality model.
...
processStartValues <- function(object, coefNames, ax, bx, kt, b0x, gc, 
  ...
  if (N > 0) {    
    ...
    for (i in 1:N) {
      if (is.function(object$periodAgeFun[[i]])) {        
        ...
        #kt
        for (t in as.character(years)) {
          ind <- which(coefNames == paste("Mult(factor(x), ., inst = ", inst, ").factor(t)", t, sep = ""))
          if (length(ind) > 0) startCoef[ind] <- kt[i,t]                
        }        
      }            
    }
  ...
  if ( !is.null(object$cohortAgeFun) == TRUE ) {
    ...
    if (is.function(object$cohortAgeFun)) {
      ...
      #b0x
      for (x in as.character(ages)) {          
        ind <- which(coefNames == paste("Mult(., factor(c)).factor(x)",x, sep = ""))
        if (length(ind) > 0) startCoef[ind] <- b0x[x]
      }        
      #gc
      for (k in as.character(cohorts)) {
        ind <- which(coefNames == paste("Mult(factor(x), .).factor(c)", k, sep = ""))
        if (length(ind) > 0) startCoef[ind] <- gc[k]                
      }      
    }
  }
