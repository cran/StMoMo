...
#' @param nsim number of sample paths to simulate from each bootstrapped sample.
...
simulate.bootStMoMo <-function(object, nsim = 1, seed = NULL, h = 50, oxt = NULL,
  ...
  fitted <- array(NA, c(dim(tempSim$fitted)[1:2], nPath), 
                  list(dimnames(tempSim$fitted)[[1]], dimnames(tempSim$fitted)[[2]], 1:nPath))  
  if(is.null(tempSim$kt.s)){
    kt.s <- NULL
  } else {
    kt.s <- list(sim = NULL, years = tempSim$kt.s$years)
    kt.s$sim <- array(NA, c(dim(tempSim$kt.s$sim)[1:2], nPath), 
                      list(dimnames(tempSim$kt.s$sim)[[1]], dimnames(tempSim$kt.s$sim)[[2]], 1:nPath))
  }
  ...
  while (i <= nBoot){    
    ...
    if (!is.null(kt.s)) kt.s$sim[, , ((i - 1) * nsim + 1):(i * nsim)] <- tempSim$kt.s$sim  
