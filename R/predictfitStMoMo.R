...
#' provided. 
#' 
#' @param oxt optional matrix/vector or scalar of known offset to be used in the
#' prediction.
...
predict.fitStMoMo <- function(object, years, kt = NULL, gc = NULL, oxt = NULL, 
  ...
  if(object$model$N > 0){
    kt <- as.matrix(kt)
    if (ncol(kt) == 1) kt <- t(as.matrix(kt))
    if(ncol(kt) != nYears){
      stop( "Mismatch between the dimension of kt and the number of years.")
    }
    if(nrow(kt) != object$model$N){
      stop( "Mismatch between the dimension of kt and the number of age-period terms
            in the model.")
    }
  } else {
    if (!is.null(kt)){
      warning("kt argument ignored as the model doesn't have any age-period terms.")
    }
    kt <- NULL
  }
  if(!is.null(object$model$cohortAgeFun)){
    ...
    if (length(gc) != nCohorts){
      stop(paste("Mismatch between the length of gc and the number of cohorts required. 
            Required number of cohorts:", nCohorts))
    }
  } else {
    if (!is.null(gc)){
      warning("gc argument ignored as the model doesn't have an age-cohort term.")
     }
    gc <- NULL
  }
  if(is.null(oxt)){
  ...
  #compute prediction
  link <- predictLink(ax = object$ax, bx = object$bx, kt = kt, b0x = object$b0x, gc = gc,
